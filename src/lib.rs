//! # winkernl-rs
//!
//! `winkernl-rs` is a Rust library providing high-level abstractions for
//! Windows kernel-mode (Ring 0) development.
//!
//! Rather than exposing only raw FFI bindings, the crate offers safe,
//! ergonomic building blocks for common Windows kernel facilities while
//! remaining faithful to the native NT programming model.
//!
//! Its goal is to reduce repetitive WDK boilerplate through RAII and
//! idiomatic Rust abstractions without sacrificing performance,
//! flexibility, or understanding of the underlying system.
//!
//! ## Features
//!
//! The library currently provides abstractions for:
//!
//! - Windows kernel pool allocations.
//! - Physically contiguous memory.
//! - Memory Descriptor Lists (MDLs).
//! - Virtual memory read/write helpers.
//! - Generic x86-64 page table walking.
//! - Virtual-to-physical address translation.
//! - Kernel module and export resolution.
//! - `EPROCESS` traversal and process utilities.
//! - Lightweight kernel inline hooking.
//! - Raw `ntddk`, `ntifs`, and `ntstatus` bindings.
//!
//! ## Memory abstractions
//!
//! `winkernl-rs` provides several RAII wrappers around Windows kernel
//! allocation routines.
//!
//! These wrappers automatically release their underlying resources while
//! exposing APIs that feel familiar to Rust developers.
//!
//! Available allocation types include:
//!
//! - `PoolMemory<T>`
//! - `PoolMemorySlice<T>`
//! - `ContiguousMemory<T>`
//!
//! They also support common allocation patterns such as zero-initialized
//! memory and safe ownership transfer when appropriate.
//!
//! ## Memory utilities
//!
//! The crate includes reusable utilities for interacting with Windows memory
...
//! - physical memory access through pluggable backends;
//! - Memory Descriptor List (MDL) management;
//! - virtual memory read/write helpers;
//! - wildcard pattern scanning.
//!
//! ## Design philosophy
//!
//! `winkernl-rs` does **not** attempt to hide the Windows kernel programming
//! model.
//!
//! Instead, it focuses on:
//!
//! - providing safe and idiomatic Rust abstractions over common kernel
//!   primitives;
//! - leveraging RAII to manage kernel resources automatically;
//! - reducing repetitive boilerplate;
//! - exposing reusable generic components that can operate across different
//!   Ring 0 environments;
//! - remaining as close as possible to the native NT APIs;
//! - introducing little to no runtime overhead whenever possible.
//!
//! ## Safety
//!
//! Kernel programming inherently requires operations that cannot always be
//! verified by Rust's type system.
//!
//! APIs that rely on caller guarantees are therefore explicitly marked
//! `unsafe`, while safe abstractions are provided wherever correctness can be
//! enforced by the library.
//!
//! ## Scope
//!
//! The library is intended to be usable from a wide range of Ring 0
...
//! - research projects.
//!
//! Rather than replacing the Windows kernel APIs, `winkernl-rs` provides
//! reusable, ergonomic building blocks that remain transparent and faithful
